...
exMsvsData1 <- exMsvsData %>%
  ...
  droplevels() %>% 
    mutate(cum_sum = cumsum(totalNumberOfPlugs)) %>% 
  mutate(Date = as.Date(Date, "%m %d %Y"))  # pivot_longer(cols = 17:25,
  #              names_to = "speciesSurvival",
  #              values_to = "1MoSurvivalBySpecies") %>% 
  #   pivot_longer(cols = 22:30,
  #              names_to = "12MospeciesSurvival",
  #              values_to = "12MoSurvivalBySpecies")
  # 
