...
fish <- fish1 %>% 
  select(date, site, transect, reviewer1SpeciesCm,reviewer1SpeciesSc,reviewer1FullSciName, reviewer1Count) %>%
  ...
  summarise(count = sum(reviewer1Count), n = max(n), abundance = ((count/n)*100))
...
fishStack1 <- ggplot(fish, aes(x = site, y = abundance, fill = class, group = reef))+
...
fishStack1 <- ggplot(fish, aes(x = site, y = reviewer1Count, fill = reviewer1FullSciName)) + 
