...
#turning a bunch of the columns into factors
...
fish1 <- fish1 %>% 
  ...
  rename("species" = "reviewer1FullSciName")
...
fish <- fish1 %>% select(date, site, species, reviewer1Count) %>%
group_by(date, site) %>% 
  add_count() %>% 
  ungroup() %>% 
...
fishY1 <- fish %>% 
  dplyr::select(date, timePoint, year, reef, site, species, count, n, abundance) %>% 
