...
###
***
This is the analysis pipeline to analyze the community composition from fish video transects. Six experimental 10m x 10m plots were established. Three transects of video were captured per site three times 1 month post-outplant and again, three more times 12 months post-outplant. Each transect was swam from the ledge to the hardpan, and 5 minutes elapsed between transects to allow for fish communities to return post diver disturbance. Still frames were pulled from each video at discrete intervals (NEED TO SAY WHAT THE INTERVALS WERE). Using the open source program **Ffmpeg**, the protocol for which can be found in the ../protocols folder within this repository. Fish were identified to the lowest taxonomic level possible, if applica ble, the phase of the fish species was noted (terminal, initial, etc).
...
fish <- fish1 %>% select(date, site, reviewer1SpeciesCm,reviewer1SpeciesSc,reviewer1FullSciName, reviewer1Count) %>%
group_by(date, site) %>% 
  add_count() %>% 
  ungroup() %>% 
...
fishPlot <- subset(fish, date == c("2020-12-14", "2020-12-16"))

fishStack1 <- ggplot(fishPlot, aes(x = site, y = abundance, fill = reviewer1FullSciName)) + 
