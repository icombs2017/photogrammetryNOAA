...
fish2 <- fishCombo %>% select(date, site, treatment, reef, year, timePoint, transect, species, count) %>%
  group_by(date, site, treatment, reef, year, timePoint, transect, species) %>% 
...
fish <- fish3 %>%
  group_by(date, site, treatment, reef, year, timePoint, species) %>% 
  group_by(date, site, treatment, reef, year, timePoint) %>% 
...
distPerm <- adonis2(fishMat ~ treatment*timePoint*reef*year, data = samplesLookupPerm, permutations = 9999, method = 'bray')
distPerm
...
boxPlotData <- fishMat %>% 
   rownames_to_column(var = "samples") %>% 
    separate(col = samples, into = c("date", "site", "treatment", "reef", "year", "timePoint"), sep = "_", remove = FALSE) %>% 
  select(-c(samples)) %>% 
  gather(class, count, 7:75, factor_key = TRUE) %>% 
  as_tibble() %>% 
  mutate_at(c("date", "site", "treatment", "reef", "year", "timePoint", "class"), as.factor) %>% 
...
fishStack1 <- ggplot(boxPlotData, aes(x = site, y = abundance)) +
