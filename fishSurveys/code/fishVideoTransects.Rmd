...
fish2 <- fish1 %>% select(date, site, transect, frame, species, count) %>%
...
fish$year <- if_else(fish$date < "2021-07-20", "Y1", if_else(fish$date > "2021-07-27", "Y1", "Y2"))
fish$timePoint <- if_else(fish$date < "2021-07-27", "T0", "T12")


# Taking all of the new columns we just made and turning them all into factors
fish <- fish %>% 
  mutate_at(c("treatment", "reef", "year", "timePoint"), as.factor)

fishBase <- fish %>%
  select(date, site, reef, treatment, year, timePoint, species, count, n, avgCount, totalObservations, abundance) %>% 
  filter(timePoint == "T0") %>% 
  # na.omit(species) %>% 
  droplevels()
...
distPerm <- adonis2(fishMat ~ treatment*year*reef, data = samplesLookupPerm, permutations = 9999, method = 'bray')
distPerm
...
myColors <- paletteer_c("pals::ocean.balance", 17, direction = 1 )
...
vpiStack1Y1 <- ggplot(boxPlotDataY1, aes(x = site, y = abundance)) +
                      geom_bar(aes(fill = class), position = position_stack(), stat = 'identity', color = 'black', alpha = 0.8) +
                      ylim(0,100) +
                      labs(y = "Relative Abundance (%)", fill = "Class", title = "Relative Species Abundance > 5% for baseline NOAA Sites")+
...
# THIS WILL BE USED WHEN WE GET MORE DATA, FOR NOW, COMMENTED OUT
...
<!-- ```{r, graph, include = TRUE, results = FALSE} -->
...
<!-- ``` -->
<!-- ![Figure](../figures/baselineFishSurveyPlot.png) -->
