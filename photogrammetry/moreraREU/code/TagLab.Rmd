...
pacman::p_load("ggplot2","officer","ggpubr", "rcompanion", "RColorBrewer", "patchwork", "magrittr","reshape2", "stringr", "plyr", "dplyr", "flextable", "tidyr", "tibble", "vegan", "paletteer","tidyverse")
```
...
tagPerm <- adonis(formula = count ~ Date*Area, data = tagData3, method = "euclidian", permutations = 9999)
##^^ problem is maybe Date is factor and Area is numeric? but then I dont think I use permanova bc of # of variables

kruskal.test(abundance ~ Date, data = tagData3)


oneYear <- subset(tagData3, time = "")
##^^ idk what the point of this is or what I need to change 
...
myColors <- paletteer_c("ggthemes::Blue-Teal", 7, direction = 1)
...
tagStack1 <- ggplot(tagData2, aes(x = Date, y = abundance, fill = Species))+
                      geom_bar(stat = 'identity', color = 'black', position = position_stack(reverse = TRUE), alpha = 0.8)+
                      ylim(0,100)+
                      labs(y = "Relative Abundance (%)", fill = "Class", title = "Relative Species Abundance (>1%) Changes Over 3 Years")+
