...
pacman::p_load("ggplot2", "officer", "ggpubr", "rcompanion", "RColorBrewer", "patchwork", "magrittr", "reshape2", "stringr", "plyr", "dplyr", "flextable", "tidyr", "tibble", "vegan", "forcats", "readr", "metafolio", "FSA", "viridis", "paletteer", 'tidyverse', "ecodist", "glue", "paletteer")
...
vpiData$timePoint <- factor(vpiData$timePoint, levels(vpiData$timePoint)[c(3,1,2)])
...
myColors <- check_palette("PRGn", 10, direction = 1 )
...
myColors <- c("#d2232a", "#ca581f", "#f26522", "#85b034", "#00ae9d", "#78779e","#FFC95C", "#007b41", "#0054a6", "#352f6d", "#34900a", "#48000a", "#a90aaa","#cad345","#dac111","#100aaa")
...
vpiStack1Y1 <- ggplot(vpi1, aes(x = timePoint, y = abundance)) +
                      geom_bar(aes(fill = class), position = position_stack(reverse = FALSE), stat = 'identity', color = 'black', alpha = 0.8) +
                      ylim(0,100) +
                      labs(y = "Relative Abundance (%)", fill = "Class", title = "Relative Species Abundance >1% Over 3 Years", x = "Time Point" )+
  scale_x_discrete(labels=c("baseline", "12 Month","24 Month"))
...
boxPlotDataY2 <- vpiMat1 %>% 
   rownames_to_column(var = "samples") %>% 
    separate(col = samples, into = c("date", "timePoint", "year", "site", "treatment", "reef"), sep = "_", remove = FALSE) %>%
  select(-c(samples)) %>% 
  gather(class, count, 7:21, factor_key = TRUE) %>% 
  as_tibble() %>% 
