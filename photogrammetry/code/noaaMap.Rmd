...
sites$Reef = factor(sites$Reef, levels = levels(sites$Reef)[c(1,2,3)])

fknmsBounds = read.csv("../data/shp/fknms_py.csv", header = TRUE)
fknmsBounds = readOGR(dsn = "../data/shp/FKNMS_Boundary")
...
floridaMap = ggplot() +
  ...
  #geom_polypath(data = fknmsBounds, aes(x = X, y = Y, color = POLY_ID, group = POLY_ID), fill = NA) +
