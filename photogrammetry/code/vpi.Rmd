...
vpiPre$class <- as.factor(vpiPre$class)
vpiPre1 <- vpiPre %>% filter(abundance > 2) %>% droplevels()
...
Conducting a few normality tests to determine the distribution of the data. And then running the appropriate statistical test. 

```{r}

summaryStats <- vpiPre %>% select(class, abundance) %>% group_by( class) %>% summarise(mean(abundance)) %>% arrange(desc(`mean(abundance)`)) %>% as_tibble()
...
kruskal.test(abundance ~ site, data = vpiPre)
